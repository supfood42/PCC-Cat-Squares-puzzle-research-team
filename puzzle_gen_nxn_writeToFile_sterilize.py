...
def canonical_piece(piece):
    """Return the smallest rotation of a piece for duplicate comparison."""
    rotations = [tuple(rotate_piece(piece, i)) for i in range(4)]
    return min(rotations)


def has_duplicate_piece(grid):
    """Return True if any two pieces are identical under rotation."""
    seen = set()
    for row in grid:
        for piece in row:
            key = canonical_piece(piece)
            if key in seen:
                return True
            seen.add(key)
    return False
...
for _ in range(total_puzzles):
    while True:
        ...
        if not STERILIZE or not has_duplicate_piece(solved):
            break
