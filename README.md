...
1. Run puzzle_gen_nxn_writeToFile_sterilize.py
2. Enter puzzle size: size of puzzle you want (nxn)
3. Number of puzzles: total number of puzzles in dataset
...
4. Press enter
###### (Tip: you can drag the file from the sidebar into the terminal to insert the filepath)


## Plan B (to limit number of threads)
1. In a terminal, go to the project folder and enter the file path of the compilled puzzle_solver_rotational_unique.exe
2. Press space
3. Enter the filepath of the dataset to be solved
   (optional)
...
If it appears stuck, then you're lucky: it's having a hard time finding solutions in this puzzle. A Unicorn puzzle requires the algo to go through all possible combinations, so the linger it takes, the less solutions there are!

For 6x6, a puzzle could take hours. As long as the CPU is computing, it is not stuck.

When the program finds the unicorn, it will stop and report the stats in the terminal. Copy it in a safe place. Make sure to not power off or you will lose it forever.
